...
class Text2(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F,
    ...
    companion object {
        ...
        fun defaultClient(): Text {
            ...
            text.displayString.set("B§flackSense")
            text.shadow.set(true)
            text.fontValue.set(Fonts.font40)
            text.setColor(Color(0, 111, 255,100))
    ...
    private val colorModeValue = ListValue("Text-Color", arrayOf("Custom", "Random", "Rainbow","Rainbow2", "Rainbow3","CustomRainbow"), "Flux")
    ...
    private fun getReplacement(str: String): String? {
        ...
        return when (str) {
            ...
            "clientVersion" -> "b${LiquidBounce.CLIENT_REAL_VERSION}"
    ...
    override fun drawElement(partialTicks: Float): Border? {
        ...
        RainbowFontShader.begin(colorMode == "Rainbow", if (rainbowX.get() == 0.0F) 0.0F else 1.0F / rainbowX.get(), if (rainbowY.get() == 0.0F) 0.0F else 1.0F / rainbowY.get(), System.currentTimeMillis() % 10000 / 10000F).use {
            fontRenderer.drawString(displayText, 0F,0F, when {
                colorMode.equals("Rainbow", ignoreCase = true) -> 0
                //    else color, shadow.get())
                else -> color
            }, shadow.get())
        }
        if (editMode && mc.currentScreen is GuiHudDesigner && editTicks <= 40)
            fontRenderer.drawString("_", fontRenderer.getStringWidth(displayText) + 2F,
                0F, if (colorMode == "Rainbow") ColorUtils.rainbow(400000000).rgb else color, shadow.get())
