...
class TabGUI(x: Double = 5.0, y: Double = 25.0) : Element(x = x, y = y) {
    ...
    override fun drawElement(partialTicks: Float): Border {
        ...
        if (borderValue.get()) {
            ...
            RenderUtils.drawRoundedCornerRect(1F, 0F, width.get(), guiHeight,4F, backgroundColor.rgb)
        }
    ...
    private inner class Tab(val tabName: String) {
        ...
        fun drawTab(
            ...
            if (borderValue.get()) {
                ...
                RenderUtils.drawRoundedCornerRect(x - 1F, y - 1F, x + menuWidth - 2F, y + menuHeight - 1F,4F, backgroundColor)
            }

            RenderUtils.drawRoundedCornerRect(x - 1.toFloat(), y + itemY - 1, x + menuWidth - 2F, y + itemY + tabHeight.get() - 1,4F, color)
            GlStateManager.resetColor()
