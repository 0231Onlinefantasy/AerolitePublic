...
public final class BotKiller extends Module {
    ...
    private static final List<EntityPlayer> removedBots = new ArrayList<>();
    private final MSTimer sendTimer = new MSTimer();
    ...
    private static void displayRemoveTip() {
        ClientUtils.INSTANCE.displayChatMessage(("§8[§c§lBotKiller§r§8] §eRemoved " + removedBots.size() + " Bots"));
    }

    @EventTarget
    public void onUpdate(UpdateEvent updateEvent) {
        if(sendTimer.hasTimePassed(3000)) {
            if(removedBots.size() != 0) displayRemoveTip();
            removedBots.clear();
            sendTimer.reset();
        }
...
//    }
    ...
    public static boolean isBot(final EntityLivingBase entity) {
        if(modeValue.get().equals("Normal")) {
            ...
            final BotKiller antiBot = (BotKiller) LiquidBounce.moduleManager.getModule(BotKiller.class);
            ...
            if (antiBot.tabValue.get()) {
                ...
                if (targetName != null) {
                    for (final NetworkPlayerInfo networkPlayerInfo : mc.getNetHandler().getPlayerInfoMap()) {
                        final String networkName = ColorUtils.INSTANCE.stripColor(EntityUtils.INSTANCE.getName(networkPlayerInfo));
            ...
            if (antiBot.duplicateInTabValue.get()) {
                if (mc.getNetHandler().getPlayerInfoMap().stream()
                        .filter(networkPlayer -> entity.getName().equals(ColorUtils.INSTANCE.stripColor(EntityUtils.INSTANCE.getName(networkPlayer))))
                        .count() > 1)
                    return true;
            }
