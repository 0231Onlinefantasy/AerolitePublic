...
@ModuleInfo(name = "KickCheck", category = ModuleCategory.VISUAL)
class KickCheck : Module() {
    ...
    private var translat = TransUtils(0f , 0f)
    ...
    fun onRender2D(e: Render2DEvent) {
        val x = RenderUtil.width() / 2f + xValue.get()
        val y = RenderUtil.height() / 2f + yValue.get()
        val width = x + 100
        ...
        // TooMany Packets
        if (packetsCheck.get()) {
            Fonts.gs40.drawString(
                "TooMany Packets",
                x + (100f - Fonts.gs40.getStringWidth("TooMany Packets")) / 2f,
                y,
        ...
        if (vanillaCheck.get()) {
            Fonts.gs40.drawString(
                "Vanilla Kick",
                x + (100f - Fonts.gs40.getStringWidth("Vanilla Kick")) / 2f,
                y + 35f,
                Color(255, 255, 255, 200).rgb,
                false
            )
