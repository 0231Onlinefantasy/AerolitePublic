...
class AutoSoup : Module() {
    ...
    private val notiValue = BoolValue("NotiInfo", false)
    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        if (mc.thePlayer.health <= healthValue.get() && soupInHotbar != -1) {
            ...
            if (notiValue.get()) LiquidBounce.hud.notifications.add(Notification("AutoSoup", "Popped ${poppedCount} soup!", NotifyType.INFO))
            timer.reset()
            return
        }
