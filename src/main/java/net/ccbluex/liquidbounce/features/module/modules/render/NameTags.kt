    ...
    private fun renderNameTag(entity: EntityLivingBase, tag: String) {
        ...
        when (modeValue.get().lowercase()) {
            ...
            "liquid" -> {
                ...
                val distanceText = if (distanceValue.get()) "§f [§f${mc.thePlayer.getDistanceToEntity(entity).roundToInt()}§f]" else ""
                val pingText = if (pingValue.get() && entity is EntityPlayer) (if (ping > 200) "§f" else if (ping > 100) "§f" else "§f") + ping + "ms §f" else ""
                val healthText = if (healthValue.get()) "§f [§f" + entity.health.toInt() + "§c❤§f]" else ""
                val botText = if (bot) " §f[§f§lBot§f]" else ""
