...
@ModuleInfo(name = "FastStairs", category = ModuleCategory.MOVEMENT)
class FastStairs : Module() {

    private val modeValue = ListValue("Mode", arrayOf("Step", "NCP", "AAC3.1.0", "AAC3.3.6", "AAC3.3.13", "AAC4Test", "AAC5"), "NCP")
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (getBlock(blockPos) is BlockStairs && !walkingDown) {
            ...
            if (modeValue.get().equals("AAC4Test")) {
                mc.timer.timerSpeed = 1.025F
            }
            if (modeValue.get().equals("AAC5")) {
