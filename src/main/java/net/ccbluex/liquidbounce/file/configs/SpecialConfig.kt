...
class SpecialConfig(file: File) : FileConfig(file) {
    ...
    override fun loadConfig(config: String) {
        ...
        GuiAltManager.randomAltField.text = "A%n%sero"
        useGlyphFontRenderer = false
        useAwtFontRender = false
        ...
        if (json.has("use-AwtFont-fontrenderer")) {
            useAwtFontRender = json.get("use-AwtFont-fontrenderer").asBoolean
        }
    ...
    override fun saveConfig(): String {
        ...
        json.addProperty("use-AwtFont-fontrenderer", useAwtFontRender)
