...
class FileManager : MinecraftInstance() {
    ...
    val fontsDir = File(dir, "Unlessf")
    val configsDir = File(dir, "CFG")
    val soundsDir = File(dir, "ClickSound")
    val legacySettingsDir = File(dir, "legacy-settings")
    val capesDir = File(dir, "Unlessc")
    val accountsConfig = AccountsConfig(File(dir, "Accounts.json"))
    val friendsConfig = FriendsConfig(File(dir, "FED.json"))
    val xrayConfig = XRayConfig(File(dir, "X-ray.json"))
    val hudConfig = HudConfig(File(dir, "HUDGUI.json"))
