...
public class AeroliteStyle extends Style {
    ...
    private Color getCategoryColor(ModuleCategory categoryName) {
        if (categoryName.equals(ModuleCategory.COMBAT)) {
            return new Color(231, 75, 58, 175);
        }
        if (categoryName.equals(ModuleCategory.PLAYER)) {
            return new Color(142, 69, 174, 175);
        }
        if (categoryName.equals(ModuleCategory.MOVEMENT)) {
            return new Color(46, 205, 111, 175);
        }
        if (categoryName.equals(ModuleCategory.RENDER)) {
            return new Color(76, 143, 200, 175);
        }
        if (categoryName.equals(ModuleCategory.WORLD)) {
            return new Color(233, 215, 100, 175);
        }
        if (categoryName.equals(ModuleCategory.MISC)) {
        ...
        if (categoryName.equals(ModuleCategory.CLIENT)) {
            return new Color(190, 190, 190, 175);
        } else return ClickGUIModule.generateColor();
    }

    @Override
    public void drawPanel(int mouseX, int mouseY, Panel panel) {
        RenderUtils.drawRect((float) panel.getX() - 3, (float) panel.getY() - 1, (float) panel.getX() + panel.getWidth() + 3,
                panel.getY() + 22 + panel.getFade(), getCategoryColor(panel.getCategory()).getRGB());
        ...
        Fonts.gs40.drawString(panel.getName(), panel.getX() + 2, panel.getY() + 6, Integer.MAX_VALUE);
    }

    @Override
    public void drawDescription(int mouseX, int mouseY, String text) {
        int textWidth = Fonts.gs35.getStringWidth(text + "Description:");

        RenderUtils.drawRect(mouseX + 9, mouseY, mouseX + textWidth + 14, mouseY + Fonts.gs35.FONT_HEIGHT + 3, new Color(26, 26, 26).getRGB());
        GlStateManager.resetColor();

        Fonts.gs35.drawString("Description:" + text, mouseX + 12, mouseY + (Fonts.gs35.FONT_HEIGHT / 2), Integer.MAX_VALUE);
    }
    ...
    public void drawButtonElement(int mouseX, int mouseY, ButtonElement buttonElement) {
        ...
        GlStateManager.resetColor();
        Fonts.gs35.drawString(buttonElement.getDisplayName(), buttonElement.getX() + 3,
    ...
    public void drawModuleElement(int mouseX, int mouseY, ModuleElement moduleElement) {
        ...
        Gui.drawRect(moduleElement.getX() + 1, moduleElement.getY() + 1, moduleElement.getX()
                ...
                new Color(getCategoryColor(moduleElement.getModule().getCategory()).getRed(), getCategoryColor(moduleElement.getModule().getCategory()).getGreen(), getCategoryColor(moduleElement.getModule().getCategory()).getBlue(), moduleElement.slowlyFade), moduleElement.hoverTime).getRGB());

        final int guiColor = ClickGUIModule.generateColor().getRGB();

        GlStateManager.resetColor();
        Fonts.gs35.drawString(moduleElement.getDisplayName(), moduleElement.getX() + 3,
                moduleElement.getY() + 7, Integer.MAX_VALUE);

        final List<Value<?>> moduleValues = moduleElement.getModule().getValues();

        if (!moduleValues.isEmpty()) {
            Fonts.gs35.drawString("+", moduleElement.getX() + moduleElement.getWidth() - 8,
            ...
            if (moduleElement.isShowSettings()) {
                ...
                for (final Value value : moduleValues) {
                    ...
                    if (value instanceof BoolValue) {
                        ...
                        float textWidth = Fonts.gs35.getStringWidth(text);
                        ...
                        GlStateManager.resetColor();
                        Fonts.gs35.drawString(text, moduleElement.getX() + moduleElement.getWidth() + 6,
                        ...
                        RenderUtils.drawRect(moduleElement.getX() + moduleElement.getWidth() + 4, yPos + 2, moduleElement.getX() +
                                moduleElement.getWidth() + moduleElement.getSettingsWidth(), yPos + 14, new Color(26, 26, 26).getRGB());
                        GlStateManager.resetColor();
                        Fonts.gs35.drawString("§e" + text, moduleElement.getX() + moduleElement.getWidth() + 6, yPos + 4, 0xffffff);
                        Fonts.gs35.drawString(listValue.openList ? "-" : "+", (int) (moduleElement.getX() + moduleElement.getWidth() +
                        ...
                        for (final String valueOfList : listValue.getValues()) {
                            final float textWidth2 = Fonts.gs35.getStringWidth(">" + valueOfList);
                            ...
                            if (listValue.openList) {
                                ...
                                GlStateManager.resetColor();
                                Fonts.gs35.drawString(">", moduleElement.getX() +
                                        moduleElement.getWidth() + 6, yPos + 4, Integer.MAX_VALUE);
                                Fonts.gs35.drawString(valueOfList, moduleElement.getX() + moduleElement.getWidth() + 14,
                        ...
                        String text = value.getName() + "§f: §b" + round(floatValue.get());
                        float textWidth = Fonts.gs35.getStringWidth(text);
                        ...
                        GlStateManager.resetColor();
                        Fonts.gs35.drawString(text, moduleElement.getX() + moduleElement.getWidth() + 6, yPos + 4, 0xffffff);
                        yPos += 22;
                    } else if (value instanceof IntegerValue) {
                        IntegerValue integerValue = (IntegerValue) value;
                        String text = value.getName() + "§f: §a" + (value instanceof BlockValue ? BlockUtils.getBlockName(integerValue.get()) + " (" + integerValue.get() + ")" : integerValue.get());
                        float textWidth = Fonts.gs35.getStringWidth(text);
                        ...
                        } else if (fontRenderer == Fonts.gs35)
                            displayString = "Font: GoogleSans-35";
                        else {
                        ...
                        assert Fonts.gs35 != null;
                        Fonts.gs35.drawString(displayString, moduleElement.getX() + moduleElement.getWidth() + 6, yPos + 4, Color.WHITE.getRGB());
                        int stringWidth = Fonts.gs35.getStringWidth(displayString);
